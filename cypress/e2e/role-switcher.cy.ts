describe("Role switcher updates document library", () => {
  beforeEach(() => {
    cy.intercept("GET", "/api/docs*", { fixture: "doc-list-viewer.json" }).as(
      "getDocs",
    );
  ...
  it("switching from viewer to data-analyst triggers doc refetch", () => {
    cy.intercept("GET", "/api/docs*", { fixture: "doc-list-analyst.json" }).as(
      "getDocsAnalyst",
    );
  ...
  it("controls bar shows Role and Policy Engine labels", () => {
    ...
    cy.contains("Policy Engine").should("be.visible");
  });
});
